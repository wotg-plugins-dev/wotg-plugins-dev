new Wotg.Plugins.Simple({
	...
	//Count
	Wotg.Card.Markup.MPack.markup.children[11].rect = [146, 37, 20, 25];
	Wotg.Card.Markup.MPack.markup.children[11].fontsize = 18;
	//Power
	...
	Wotg.Card.Markup.MHand.markup.children[10].children[0].rect = [2, 2, 100, 30];
	//Cost
	Wotg.Card.Markup.MHand.markup.children[9].rect = [0, 0, 24, 26];
	...
	declare( 'Wotg.Card.Views.MHand', Wotg.Card.View, {
		...
		redraw: function () {
			...
			if (model.getProperty('type') == 'order') {
				...
				var power;
				power = model.getProperty('power');
				if (model.getProperty('type') == 'platoon'){
					if (model.getProperty('isDefense')){
						power = model.getProperty('defense')
					}
				}
				this.setValue('Power', power + "/" + model.getProperty('toughness')+ increase);
